...
- **A per-mutant `RUN_ERROR` is the same shape, and the plugin refuses the whole
  ...
  the scoped/isolated comparison the template describes, not a verdict.
...
`21.5.26`, and the marker above is that release's digest, reconciled bullet by
bullet on 2026-08-18. **The pins must move together**; a settings-only bump has
