...
node's grant of each accepted registration (`Subscription.subId()`) rather than a notification,
because a cycle drawn from a list's programs and mints would never see one. `W1-A` renders no
