...
None yet — the initial baselines are untriaged debt, not accepted
equivalents. `borsh` seeded empty: keep it that way.

## Untriaged debt

The baselines were seeded 2026-07-18 with the pre-existing
`SURVIVED`/`NO_COVERAGE` population. The bulk sits in `tx`, where the
widening from the skeleton/lookup allowlist to the full `tx` and
`accounts.lookup` packages exposed the transaction/instruction builder
classes as uncovered.
